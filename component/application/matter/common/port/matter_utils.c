...
#define MATTER_SECURE_CONTEXT_STACK_SIZE (4 * configMINIMAL_SECURE_STACK_SIZE)
...
int matter_get_signature(uint8_t *pub_buf, size_t pub_size, const unsigned char * msg, size_t msg_size , unsigned char * signature)
{
    ...
    int result = matter_secure_dac_init_keypair(pub_buf, pub_size);

    if (result != 0) {
        DiagPrintf("Error: %s DAC init failed, result=%d \n\r", __FUNCTION__, result);
        return result;
    }
...
int matter_ecdsa_sign_msg(const unsigned char * msg, size_t msg_size , unsigned char * signature)
{
    ...
    int result = matter_secure_ecdsa_sign_msg(MATTER_OPKEY_KEY_TYPE ,msg, msg_size, signature);

    if (result != 0) {
        DiagPrintf("ERROR: %s get signature failed, result=%d \n\r", __FUNCTION__, result);
    }
...
int matter_get_publickey(uint8_t * pubkey, size_t pubkey_size)
{
    ...
    int result = matter_secure_get_opkey_pub(pubkey, pubkey_size);
...
size_t matter_gen_new_csr(uint8_t *out_csr, size_t csr_length)
{
    ...
    int result = matter_secure_new_csr(out_csr, csr_length);

    if(result <= 0) {
        DiagPrintf("ERROR: %s create csr failed, return=%d \n\r", __FUNCTION__, result);
    }
...
int matter_serialize(uint8_t *output_buf, size_t output_size)
{
    ...
    int result = matter_secure_serialize(output_buf, output_size);

    if(result != 0) {
        DiagPrintf("ERROR: %s serialize failed, result=%d \n\r", __FUNCTION__,result);
    }
...
int matter_deserialize(uint8_t *buf, size_t size)
{
    ...
    int result = matter_secure_get_opkey(buf, size);

    if(result != 0) {
        DiagPrintf("ERROR: %s decrypt failed, result=%d \n\r", __FUNCTION__,result);
    }

    return result;
}
#endif /* (defined(CONFIG_MATTER_SECURE) && (CONFIG_MATTER_SECURE == 1))*/
