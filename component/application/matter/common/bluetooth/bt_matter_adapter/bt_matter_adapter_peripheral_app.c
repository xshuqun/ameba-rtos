...
void bt_matter_adapter_app_set_adv_data(void)
{
	...
	//DiagPrintf("Device name: \"%s\" (BD Address %02X:%02X:%02X:%02X:%02X:%02X) \n\r",
...
void bt_matter_adapter_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    if (chip_blemgr_callback_func && chip_blemgr_callback_data)
        ...
        DiagPrintf("chip_blemgr_callback_func is NULL\r\n");
    return;
#else
    memcpy(&gap_msg, &p_gap_msg->u.param, sizeof(p_gap_msg->u.param));
	//DiagPrintf("bt_matter_adapter_app_handle_gap_msg: subtype %d\n\r", p_gap_msg->subtype);
...
T_APP_RESULT bt_matter_adapter_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
	...
	//DiagPrintf("GAP_MSG_LE_DATA_LEN_CHANGE_INFO: conn_id %d, tx octets 0x%x, max_tx_time 0x%x", p_data->p_le_data_len_change_info->conn_id, p_data->p_le_data_len_change_info->max_tx_octets,  p_data->p_le_data_len_change_info->max_tx_time);
                ...
                case BTCONFIG_WRITE_V1:
                    {
						...
						DiagPrintf("Calling BLEManager here==========\n");
						// call blemanagerimpl function
                        if (chip_blemgr_callback_func && chip_blemgr_callback_data)
