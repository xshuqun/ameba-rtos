                                            ...
                                            uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    {
    default:
        DiagPrintf("bt_matter_adapter_service_attr_read_cb, Attr not found, index %d", attrib_index);
    ...
    case BT_MATTER_ADAPTER_SERVICE_CHAR_V1_READ_WRITE_INDEX:
        {
            ...
            *p_length = bt_matter_adapter_char_read_len;
			//DiagPrintf("[BT_MATTER_ADAPTER] Read %d\n\r", bt_matter_adapter_char_read_len);
        }
        break;
...
{
	DiagPrintf("[BT_MATTER_ADAPTER] Write %d\n\r", length);
    TBTCONFIG_CALLBACK_DATA callback_data;
	T_APP_RESULT  cause = APP_RESULT_SUCCESS;
                                     ...
                                     uint16_t cccbits)
{
    ...
    //DiagPrintf("simp_ble_service_cccd_update_cb: index = %d, cccbits 0x%x\r\n", index, cccbits);
    switch (index)
    {
