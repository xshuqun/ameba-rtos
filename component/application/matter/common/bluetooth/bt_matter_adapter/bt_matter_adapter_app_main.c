...
// extern uint8_t airsync_specific; //undefined
 ...
 * @return void
 */
// extern void gap_config_hci_task_secure_context(uint32_t size); //undefined
void bt_matter_adapter_stack_config_init(void)
{
    ...
    // gap_config_hci_task_secure_context (280); //undefined
}
/**
...
void bt_matter_adapter_app_le_gap_init_chip(void)
{
    ...
    if (gen_addr)
    {
        if (le_gen_rand_addr(GAP_RAND_ADDR_STATIC, random_addr.bd_addr) == GAP_CAUSE_SUCCESS)
        {
            ...
            DiagPrintf("bd addr: %02x:%02x:%02x:%02x:%02x:%02x\r\n", 
