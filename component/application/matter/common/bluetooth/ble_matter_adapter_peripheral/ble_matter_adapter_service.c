...
void ble_matter_adapter_ble_service_callback(uint8_t event, void *data)
{
	...
	case RTK_BT_GATTS_EVT_INDICATE_COMPLETE_IND:
	{
		...
		if (RTK_BT_OK == p_ind_ind->err_code)
		{
			...
			{
				BLE_MATTER_APP_SEND_COMPLETE_CB_ARG *send_complete_msg_matter = (BLE_MATTER_APP_SEND_COMPLETE_CB_ARG *)os_mem_alloc(0,sizeof(BLE_MATTER_APP_SEND_COMPLETE_CB_ARG));
