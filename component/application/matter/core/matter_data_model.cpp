...
void Endpoint::enableEndpoint()
{
    ...
    if (status == CHIP_NO_ERROR)
    {
        ...
        enabled = true;
        if(endpointId > 1) // to set the bridged device reachableness status and label
        {
            char name[18] = {0};
            sprintf(name, "Bridged Device %02d", endpointId);
            chip::DeviceLayer::PlatformMgr().LockChipStack();
            if(Status::Success != Clusters::BridgedDeviceBasicInformation::Attributes::Reachable::Set(endpointId, true))
                ChipLogError(DeviceLayer, "Failed to set Bridged Device Reachable");
            if(Status::Success != Clusters::BridgedDeviceBasicInformation::Attributes::NodeLabel::Set(endpointId, CharSpan::fromCharString(name)))
                ChipLogError(DeviceLayer, "Failed to set Bridged Device Node Lable");
            chip::DeviceLayer::PlatformMgr().UnlockChipStack();
        }
        return;
    }
...
{
    if (endpointCount < CHIP_DEVICE_CONFIG_DYNAMIC_ENDPOINT_COUNT)
    {
        ...
        {
            if(nextEndpointId > 1)
                endpoint.setParentEndpointId(1);
            else
                endpoint.setParentEndpointId(0);
        }
        ...
        endpointCount++;
        nextEndpointId++;
        if (nextEndpointId >= CHIP_DEVICE_CONFIG_DYNAMIC_ENDPOINT_COUNT)
            nextEndpointId = 2;
    ...
    {
        ChipLogError(DeviceLayer, "Reached maximum number of Dynamic Endpoint!");
        return kInvalidEndpointId; //invalid endpoint id
    }
}
...
void Node::removeEndpoint(chip::EndpointId endpointId)
{
    if(endpointId < 2)
    {
        ChipLogError(DeviceLayer, "Not allowed to delete Root and Aggregator Endpoints!");
        return;
    }
