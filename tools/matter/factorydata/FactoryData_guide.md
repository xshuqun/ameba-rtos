...
Make sure your firmware is built with `DCONFIG_ENABLE_AMEBA_FACTORY_DATA` enabled in the `Makefile.include.matter.km4.gen` under`sdk-ameba-rtos_v1.0a/component/application/matter/project/amebadplus/`
...
Edit `gen-certs.sh` under `sdk-ameba-rtos_v1.0a/tools/matter`
...
Navigate to below directory, if matter's environment is activated, deactivate it
    ...
    cd sdk-ameba-rtos_v1.0a/tools/matter
...
Flash the binary file using [AmebaImageTool.exe](tools/ameba/ImageTool/AmebaImageTool.exe)

* Choose the AmebaDplus for the device profile
* Add one section more to the last row of the table and name it `ameba_factory.bin`
* Set the Image Path to the `ameba_factory.bin` path
* Set the Start Address to `0x08400000` and the End Address to `0x08401000`  
* Flash the image
    
Note: Default address to flash `ameba_factory.bin` is 0x08400000, you may configure it using `MATTER_FACTORY_DATA` macro in `matter_utils.c`. Make sure to check for partition conflicts.
